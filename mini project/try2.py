...
class Student:
    def __init__(self, roll_number, name, student_id, face_encoding):
        ...
        self.grace_period_end = None  # To track the end of the grace period
    
    def login(self):
        if not self.is_logged_in:
            self.login_time = datetime.now()
            self.grace_period_end = self.login_time + timedelta(seconds=15)  # Set the grace period end time
    ...
    def logout(self):
        if self.is_logged_in:
            ...
            # Check if logout is within the grace period
            if self.logout_time <= self.grace_period_end:
                duration = (self.logout_time - self.login_time).total_seconds() / 60  # Convert to minutes
                self.attendance_duration += duration
                print(f"{self.name} attended for {duration:.2f} minutes (within grace period).")
            else:
                print(f"{self.name} logged out after the grace period; no attendance recorded.")
...
class OnlineClass:
    ...
    def student_logout(self, student_id):
        if student_id in self.students:
            ...
            # Record attendance details into a DataFrame only if attended
            if student.attendance_duration > 0:
