...
class Student:
    ...
    def logout(self):
        if self.is_logged_in:
            ...
            self.send_email(duration)  # Send email after logout
    ...
    def send_email(self, duration):
        sender_email = "sarthak.sharma_cs22@gla.ac.in"  # Replace with your email
        receiver_email = "sarthak.sharma_cs22@gla.ac.in"
        password = "rpoj yaxc elsz flld"  # Make sure to use an application-specific password if needed

        # Create the email content
        subject = f"Attendance Record for {self.name}"
        body = (f"Dear {self.name},\n\n"
                f"You have logged out successfully.\n"
                f"Attendance Duration: {duration:.2f} minutes.\n"
                f"Login Time: {self.login_time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                f"Logout Time: {self.logout_time.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        ...
        try:
            ...
            with smtplib.SMTP('smtp.gmail.com', 587) as server:
                ...
                print(f"Email sent to {self.name} at {receiver_email}")
